...
RSpec.describe Weather, type: :module do
  describe ".temperature_of" do
    ...
    context "when the weather API is unavailable" do
      before do
        WebMock.stub_request(:get, %r{https://api.darksky.net/forecast/\w+/18.180555,-66.749961})
          .to_return(
            status: 200,
            body: File.read(
              File.join("spec", "fixtures", "dark_sky_unavailable_response.json"),
            ),
          )
      end
      ...
      it "raises an exception" do
        expect do
          described_class.temperature_of("00601")
        end.to raise_error(Weather::DataTemporarilyUnavailable, /service is temporarily down/)
      end
    end
  end
end
